...
export const handleTournament: GameSocketHandler = (
  socket: Socket,
  io: Server
) => {
      ...
      try {
        ...
        if (!data.name || !data.hostEmail || !data.hostNickname || !data.size) {
          ...
          return socket.emit('TournamentError', {
            message: 'Missing required tournament information.',
          })
        }
        ...
        for (const key of keys) {
          ...
          if (existingTournamentData) {
            const existingTournament: Tournament = JSON.parse(
              existingTournamentData
            )
            if (
              existingTournament.name.toLowerCase() ===
                data.name.toLowerCase() &&
              (existingTournament.status === 'lobby' ||
    ...
    async (data: { inviteId: string; inviteeEmail: string }) => {
      try {
        ...
        const tournamentData = await redis.get(
          `${TOURNAMENT_PREFIX}${invite.tournamentId}`
        )
        if (!tournamentData)
          return socket.emit('TournamentInviteResponse', {
            status: 'error',
        ...
        if (tournament.status !== 'lobby') {
          return socket.emit('TournamentInviteResponse', {
            status: 'error',
            message: 'Tournament is no longer accepting players.',
          })
        }
        ...
        // Get user data
        const user: any = await getUserByEmail(inviteeEmail)
        if (!user)
          return socket.emit('TournamentInviteResponse', {
            status: 'error',
            message: 'User not found.',
        ...
        tournament.matches = createTournamentBracket(
        ...
        socket.emit('TournamentInviteResponse', {
          status: 'error',
          message: 'Failed to accept tournament invite.',
        })
      }
    }
  )
  ...
  socket.on(
    ...
    async (data: { inviteId: string; inviteeEmail: string }) => {
      try {
        ...
        socket.emit('TournamentInviteResponse', {
          status: 'error',
          message: 'Failed to decline tournament invite.',
        })
      }
    }
  )
    ...
    async (data: { inviteId: string; hostEmail: string }) => {
      try {
        ...
        if (invite.hostEmail !== hostEmail)
          return socket.emit('TournamentInviteResponse', {
            status: 'error',
            message: 'You can only cancel your own invites.',
          })
        await Promise.all([
          redis.del(`${TOURNAMENT_INVITE_PREFIX}${inviteId}`),
        ...
        socket.emit('TournamentInviteResponse', {
          status: 'error',
          message: 'Failed to cancel tournament invite.',
        })
      }
    }
  )
    ...
    async (data: { tournamentId: string; hostEmail: string }) => {
      try {
        ...
        if (!tournamentId || !hostEmail) {
          ...
          return socket.emit('TournamentStartResponse', {
            status: 'error',
            message: 'Missing info.',
          })
        }
        ...
        if (tournament.participants.length !== tournament.size) {
          ...
          return socket.emit('TournamentStartResponse', {
            status: 'error',
            message: `Tournament needs ${tournament.size} players to start.`,
          })
        }

        // Create tournament bracket with matches in WAITING state (don't start games yet)
        const matches = createTournamentBracket(
          tournament.participants,
          tournament.size
        )
        ...
        const eventData = {
          ...
          tournament,
          message:
            'Tournament started! View the bracket and wait for the host to start rounds.',
        }
    ...
    async (data: {
      ...
      try {
        ...
        if (!tournamentId || !hostEmail || round === undefined) {
          console.error('Missing required data for StartCurrentRound:', {
            tournamentId,
            hostEmail,
            round,
          })
          return socket.emit('StartCurrentRoundResponse', {
            status: 'error',
        ...
        if (tournament.hostEmail !== hostEmail) {
          ...
          return socket.emit('StartCurrentRoundResponse', {
            status: 'error',
            message: 'Only the host can start rounds.',
          })
        }
        ...
        if (currentRoundMatches.length === 0) {
          ...
          return socket.emit('StartCurrentRoundResponse', {
            status: 'error',
            message: 'No matches ready to start in this round.',
          })
        }
        ...
        for (const match of currentRoundMatches) {
          ...
          const player1Sockets =
            (await getSocketIds(match.player1.email, 'sockets')) || []
          const player2Sockets =
            (await getSocketIds(match.player2.email, 'sockets')) || []
        ...
        io.to(tournamentRoom).emit('TournamentRoundStarted', {
          ...
          message: `Round ${
            round + 1
          } has started! Players are being redirected to their matches.`,
        })

        // Send success response to host
        socket.emit('StartCurrentRoundResponse', {
          status: 'success',
          message: `Round ${round + 1} started successfully. ${
          ...
          currentMatch:
            tournament.status === 'in_progress'
              ? tournament.matches.find(
                  (m) =>
                    (m.player1?.email === playerEmail ||
                      m.player2?.email === playerEmail) &&
                    m.state === 'waiting'
                )
              : null,
        })
  ...
  socket.on(
    'JoinTournamentAsNewParticipant',
    async (data: { tournamentId: string; playerEmail: string }) => {
      try {
        const { tournamentId, playerEmail } = data
        ...
        const user: any = await getUserByEmail(playerEmail)
        if (!user)
          return socket.emit('TournamentJoinResponse', {
            status: 'error',
            message: 'User not found.',
    ...
    async (data: { tournamentId: string; playerEmail: string }) => {
      try {
        ...
        if (!isParticipant && !isHost) {
          return socket.emit('TournamentDataResponse', {
            status: 'error',
            message: 'You are not authorized to view this tournament.',
          })
        }
        ...
        socket.emit('TournamentDataResponse', {
          status: 'error',
  ...
  socket.on(
    ...
    async (data: {
      ...
      try {
        const { tournamentId, matchId, winnerEmail, loserEmail, playerEmail } =
          data
        ...
        if (!tournamentData)
          return socket.emit('TournamentMatchResponse', {
            status: 'error',
            message: 'Tournament not found.',
          })
        ...
        if (
          match.player1?.email !== playerEmail &&
          match.player2?.email !== playerEmail
        ) {
          return socket.emit('TournamentMatchResponse', {
            status: 'error',
        ...
        const loserParticipant = tournament.participants.find(
          (p) => p.email === loserEmail
        )
        if (loserParticipant) {
          loserParticipant.status = 'eliminated'

          // Send loser back to lobby
          const loserSocketIds =
            (await getSocketIds(loserEmail, 'sockets')) || []
          if (loserSocketIds.length > 0) {
            io.to(loserSocketIds).emit('TournamentPlayerEliminated', {
              tournamentId,
              matchId,
              message:
                'You have been eliminated from the tournament. Returning to lobby...',
              redirectTo: '/play',
            })
          }
        }
        ...
        const allRoundComplete = roundMatches.every(
          (m) => m.state !== 'waiting' && m.state !== 'in_progress'
        )
        ...
        if (allRoundComplete) {
          ...
          if (updatedTournament.status === 'completed') {
            ...
            const winner = updatedTournament.participants.find(
              (p) => p.status === 'winner'
            )

            // Send winner to lobby
            if (winner) {
              const winnerSocketIds =
                (await getSocketIds(winner.email, 'sockets')) || []
            ...
            const hostSocketIds =
              (await getSocketIds(updatedTournament.hostEmail, 'sockets')) || []
            if (hostSocketIds.length > 0) {
              io.to(hostSocketIds).emit('TournamentCompleted', {
                ...
                winner,
                message:
                  'Tournament completed! You can still manage the tournament.',
            ...
            const nextRoundMatches = updatedTournament.matches.filter(
              (m) => m.round === nextRound
            )
            ...
            for (const nextMatch of nextRoundMatches) {
              if (nextMatch.player1 && nextMatch.player2) {
                ...
                const player1SocketIds =
                  (await getSocketIds(nextMatch.player1.email, 'sockets')) || []
                const player2SocketIds =
                  (await getSocketIds(nextMatch.player2.email, 'sockets')) || []

                // Send both players to their next match
                const allMatchSocketIds = [
                  ...player1SocketIds,
                  ...player2SocketIds,
                ]
        ...
        socket.emit('TournamentMatchResponse', {
          status: 'success',
    ...
    async (data: { tournamentId: string; hostEmail: string }) => {
      try {
        ...
        if (tournament.status !== 'lobby') {
          return socket.emit('TournamentCancelResponse', {
            status: 'error',
            message: 'Tournament cannot be cancelled at this stage.',
          })
        }
    ...
    async (data: { tournamentId: string; playerEmail: string }) => {
      try {
        ...
        if (playerEmail === tournament.hostEmail) {
          ...
          const remainingParticipantEmails = tournament.participants.map(
            (p) => p.email
          )
          const allSocketIds: any = []
...
export async function getTournamentHostEmail(
  tournamentId: string
): Promise<string | null> {
  try {
    const tournamentData = await redis.get(
