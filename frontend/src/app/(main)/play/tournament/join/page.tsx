...
const TournamentCard = ({
  ...
  const isParticipant = tournament.participants.some(
    (p) => p.email === user?.email || p.login === user?.email
  )
    ...
    <div className="rounded-lg p-6 border border-[#2a2f3a] hover:border-[#3a3f4a] transition-all">
      ...
      <div className="mb-4">
        ...
        <div className="flex flex-wrap gap-2">
          {tournament.participants.map((participant, index) => (
            <div
              key={index}
              className="flex items-center border border-[#2a2f3a] rounded-full px-3 py-1"
            >
              <div className="w-6 h-6 rounded-full border border-[#3a3f4a] overflow-hidden mr-2">
                <Image
                  src={
                    participant.avatar
                      ? `/images/${participant.avatar}`
                      : '/avatar/Default.avif'
                  }
              ...
              <span className="text-white text-xs">
                {participant.login}
                {participant.isHost && (
                  <span className="text-blue-400 ml-1">(Host)</span>
                )}
              </span>
            </div>
          ))}
          {/* Empty slots */}
          {Array.from({
            length: tournament.maxParticipants - tournament.currentParticipants,
          }).map((_, index) => (
            <div
              key={`empty-${index}`}
              className="flex items-center border border-dashed border-[#3a3f4a] rounded-full px-3 py-1"
            >
      ...
      <div className="flex justify-end space-x-3">
        {isParticipant && (
          <button
            onClick={() =>
              (window.location.href = `/play/tournament/${tournament.tournamentId}`)
            }
...
export default function JoinTournamentPage() {
  ...
  useEffect(() => {
    ...
    const handleJoinResponse = (data: {
      ...
      if (data.status === 'success' && data.tournamentId) {
        setNotification({
          message:
            'Successfully joined tournament! Redirecting to tournament lobby...',
          type: 'success',
        })
        ...
        setNotification({
          message: data.message || 'Failed to join tournament',
          type: 'error',
        })
      }
      setIsJoining(null)
  ...
  const handleJoinTournament = (tournamentId: string) => {
    if (!user || !socket) {
      setNotification({
        message: 'Please log in to join tournaments',
        type: 'error',
      })
      return
    }
            ...
            <div className="flex items-center space-x-4">
              <div>
                ...
                <p className="text-gray-400">
                  Find and join active ping pong tournaments
                </p>
              </div>
            </div>
            <button
          ...
          {isLoading || filteredTournaments.length === 0 ? (
            <EmptyState
              ...
              onCreateTest={() => {
                ...
                setNotification({
                  message: 'Test tournament creation not implemented.',
                  type: 'error',
                })
              }}
            />
