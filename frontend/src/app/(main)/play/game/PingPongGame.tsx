...
export const PingPongGame: React.FC<PingPongGameProps> = ({
  ...
  const fetchTournamentParticipant = async (email: string) => {
    try {
      const response = await axiosGameInstance.get(
        `/tournament-participant?email=${encodeURIComponent(email)}`
      )
    ...
    if (isTournamentMode && isRemoteGame) {
      const fetchParticipantData = async () => {
        try {
          ...
          if (opponent?.email) {
            const opponentData = await fetchTournamentParticipant(
              opponent.email
            )
            if (opponentData) {
              if (isHost) {
  ...
  const safePlayer1 = {
    id:
      isTournamentMode && tournamentPlayer1Data
        ? tournamentPlayer1Data.id
        : (isRemoteGame && isHost
            ? user?.id
            : isRemoteGame
            ? opponent?.id
    ...
    name:
      isTournamentMode && tournamentPlayer1Data
        ? tournamentPlayer1Data.name
        : isRemoteGame && isHost
        ? user?.username
        : isRemoteGame
        ? opponent?.username || opponent?.name
        : user?.username,
    avatar:
      isTournamentMode && tournamentPlayer1Data
        ? tournamentPlayer1Data.avatar
        : isRemoteGame && isHost
        ? user?.avatar
        : isRemoteGame
        ? opponent?.avatar
    ...
    nickname:
      isTournamentMode && tournamentPlayer1Data
        ? tournamentPlayer1Data.nickname
        : (isRemoteGame && isHost
            ? user?.login
            : isRemoteGame
            ? opponent?.login || opponent?.nickname
            : user?.login) || 'Player 1',
    email:
      isTournamentMode && tournamentPlayer1Data
        ? tournamentPlayer1Data.email
        : isRemoteGame && isHost
        ? user?.email
        : isRemoteGame
        ? opponent?.email
  ...
  const safePlayer2 = {
    id:
      isTournamentMode && tournamentPlayer2Data
        ? tournamentPlayer2Data.id
        : (isRemoteGame && !isHost
            ? user?.id
            : isRemoteGame
            ? opponent?.id
            : player2?.id) || crypto.randomUUID(),
    name:
      isTournamentMode && tournamentPlayer2Data
        ? tournamentPlayer2Data.name
        : isRemoteGame && !isHost
        ? user?.username
        : isRemoteGame
        ? opponent?.username || opponent?.name
        : player2?.username || player2?.name,
    avatar:
      isTournamentMode && tournamentPlayer2Data
        ? tournamentPlayer2Data.avatar
        : isRemoteGame && !isHost
        ? user?.avatar
        : isRemoteGame
        ? opponent?.avatar
    ...
    nickname:
      isTournamentMode && tournamentPlayer2Data
        ? tournamentPlayer2Data.nickname
        : (isRemoteGame && !isHost
            ? user?.login
            : isRemoteGame
            ? opponent?.login || opponent?.nickname
            : player2?.login || player2?.nickname) || 'Player 2',
    email:
      isTournamentMode && tournamentPlayer2Data
        ? tournamentPlayer2Data.email
        : isRemoteGame && !isHost
        ? user?.email
        : isRemoteGame
        ? opponent?.email
  ...
  useEffect(() => {
    ...
    const handleTouchMove = (e: TouchEvent) => {
      ...
      if (
        target &&
        (target.tagName === 'CANVAS' || target.tagName === 'BUTTON')
      ) {
        e.preventDefault()
      }
    }
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      const maxWidth = isMobile() ? Math.min(containerWidth - 16, 400) : 1200
      ...
      if (isRemoteGame) {
        if (isHost) {
          if (!mobile) {
            ...
            if (
              keys.current['s'] &&
              paddle1Y.current < GAME_HEIGHT - PADDLE_HEIGHT
            ) {
              paddle1Y.current += PADDLE_SPEED
            }
          } else {
          ...
          if (!mobile) {
            ...
            if (
              ...
              paddle2Y.current < GAME_HEIGHT - PADDLE_HEIGHT
            )
              paddle2Y.current += PADDLE_SPEED
          } else {
        ...
        if (isGameHost) {
          ...
          if (
            ball.current.y <= 0 ||
            ball.current.y + BALL_SIZE >= GAME_HEIGHT
          ) {
            ball.current.dy *= -1
          }
          if (
          ...
          if (ball.current.x < -BALL_SIZE) {
            currentScores.current = {
              p1: currentScores.current.p1,
              p2: currentScores.current.p2 + 1,
            }
            ...
            currentScores.current = {
              p1: currentScores.current.p1 + 1,
              p2: currentScores.current.p2,
            }
            setScores(currentScores.current)
            resetBall(1)
            if (isRemoteGame && isGameHost && socket && gameId) {
        ...
        if (!mobile) {
          ...
          if (
            ...
            paddle1Y.current += PADDLE_SPEED
          if (
            (keys.current['arrowup'] || keys.current['↑']) &&
            paddle2Y.current > 0
          )
            paddle2Y.current -= PADDLE_SPEED
          if (
            (keys.current['arrowdown'] || keys.current['↓']) &&
            paddle2Y.current < GAME_HEIGHT - PADDLE_HEIGHT
          )
            paddle2Y.current += PADDLE_SPEED
        } else {
    ...
    if (scores.p1 >= 7 || scores.p2 >= 7) {
      ...
      if (isRemoteGame && socket && gameId) {
        if (isGameHost) {
          ...
          const winnerEmail =
            scores.p1 >= 7
              ? safePlayer1.email || user?.email
              : safePlayer2.email || opponent?.email
          const loserEmail =
            scores.p1 >= 7
              ? safePlayer2.email || opponent?.email
              : safePlayer1.email || user?.email
      ...
      if (isTournamentMode) {
        ...
        const currentUserWon =
          (isHost && scores.p1 >= 7) || (!isHost && scores.p2 >= 7)
  ...
  const handleMobilePress = useCallback(
    (which: 'p1up' | 'p1down' | 'p2up' | 'p2down', isDown: boolean) => {
                ...
                {(!isRemoteGame || isHost) && (
                  <div className="flex flex-col items-center">
                    <div className="flex gap-1 items-center mb-1">
                      <span className="w-5 h-5 flex items-center justify-center bg-gray-700 text-white rounded text-xs font-bold border border-gray-400">
                        W
                      </span>
          ...
          <div className="grid grid-cols-3 items-center gap-2 sm:gap-4 mb-4">
            ...
            <div className="flex items-center gap-2 sm:gap-3 justify-start">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-gray-400 text-xs sm:text-sm md:text-lg truncate">
            ...
            <div className="flex items-center gap-2 sm:gap-3 justify-end">
              ...
              <img
                src={
                  isRemoteGame
                    ? `${
                        safePlayer2.avatar?.startsWith('/')
                          ? safePlayer2.avatar
                          : `/images/${safePlayer2.avatar}`
                      }`
                    : `${
                        safePlayer2.avatar?.startsWith('data:')
                          ? safePlayer2.avatar
                          : safePlayer2.avatar === 'Default.avif' ||
                            safePlayer2.avatar === 'Default.svg'
                          ? `/avatar/${safePlayer2.avatar}`
                          : safePlayer2.avatar?.startsWith('/')
                          ? safePlayer2.avatar
                          : `/avatar/Default.avif`
                      }`
